...
namespace Tournaments.Sample
{
    public partial class TournamentView : Form
    {
        ...
        private List<User> teams = new List<User>();
        ...
        private void UpdateState()
        {
            ...
            if (validState)
            {
                try
                {
                    ...
                    {
                        var item = new ListViewItem(new string[] { standing.Rank.ToString(), this.teamNames[standing.Team.Id], standing.ScoreDescription });
        ...
        private void AddTeam_Click(object sender, EventArgs e)
        {
            if (this.rounds.Count == 0)
            {
                ...
                var team = new User(id, 100);
                this.teams.Add(team);
                this.teamNames.Add(id, "New Team");

                var item = new ListViewItem(new string[] { this.teamNames[team.Id], team.BotScore.ToString() });
                item.Tag = team;
        ...
        private void TeamsList_DoubleClick(object sender, EventArgs e)
        {
            if (this.TeamsList.SelectedIndices.Count != 0)
            {
                ...
                var team = (User)item.Tag;
                string name = teamNames[team.Id];
                int rating = team.BotScore.Value;

                using (EditTeamView dlg = new EditTeamView(name, rating))
                {
                    dlg.ShowDialog();
                    teamNames[team.Id] = dlg.TeamName;
                    team.BotScore = dlg.Rating;
                    item.SubItems[0].Text = teamNames[team.Id];
                    item.SubItems[1].Text = team.BotScore.ToString();
                }
        ...
        private void TeamsList_AfterLabelEdit(object sender, LabelEditEventArgs e)
        {
            ...
            var team = (User)item.Tag;
            teamNames[team.Id] = e.Label;

            if(this.teamItemsHelper.ContainsKey(team.Id))
            {
                var staleItems = this.teamItemsHelper[team.Id];
                foreach (var stale in staleItems)
                {
                    stale.SubItems[1].Text = teamNames[team.Id];
                }
            }
        ...
        private void StartNext_Click(object sender, EventArgs e)
        {
            ...
            if (round != null)
            {
                ...
                foreach (var pairing in round.Pairings)
                {
                    ...
                    {
                        var item = new ListViewItem(new string[] { teamScore.Score == null ? "" : teamScore.Score.ToString(), this.teamNames[teamScore.Team.Id] });
                        item.Tag = teamScore;
                        item.Group = group;
                        this.RoundsList.Items.Add(item);
                        this.roundItemsHelper[round].Add(item);

                        if (!this.teamItemsHelper.ContainsKey(teamScore.Team.Id))
                        {
                            this.teamItemsHelper.Add(teamScore.Team.Id, new List<ListViewItem>());
                        }

                        this.teamItemsHelper[teamScore.Team.Id].Add(item);
                    }
