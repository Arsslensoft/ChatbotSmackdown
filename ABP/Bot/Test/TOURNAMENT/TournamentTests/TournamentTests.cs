...
namespace TournamentTests
{
    ...
    public class TournamentTests
    {
        ...
        {
            IPairingsGenerator bopg = new BoilOffPairingsGenerator();
            List<User> teams = new List<User>(CreateTeams(9));
        ...
        public void BoilOffThreeTeamsIsOneRound()
        {
            ...
            List<User> teams = new List<User>(CreateTeams(3));
            List<TournamentRound> rounds = new List<TournamentRound>();
        ...
        public void BoilOffHandlesManyCompetitorsWell()
        {
            ...
            List<User> teams = new List<User>(CreateTeams(50));
            List<TournamentRound> rounds = new List<TournamentRound>();
        ...
        public void RoundRobinHandlesManyCompetitorsWell()
        {
            ...
            List<User> teams = new List<User>(CreateTeams(30));
            List<TournamentRound> rounds = new List<TournamentRound>();
        ...
        public void SingleEliminationLastPersonNeverGetsABye()
        {
            ...
            for (int i = 2; i <= 33; i++)
            {
                List<User> teams = new List<User>(CreateTeams(i));
                List<TournamentRound> rounds = new List<TournamentRound>();

                pg.LoadState(teams, rounds);

                User targetTeam = teams[teams.Count - 1];

                TournamentRound round = pg.CreateNextRound(null);

                var targetTeamPairings = from p in round.Pairings
                                         where p.TeamScores.Where(ts => ts.Team.Id == targetTeam.Id).Any()
                                         select p;
        ...
        public void SingleEliminationTieDisallowed()
        {
            ...
            List<User> teams = new List<User>(CreateTeams(2));
            List<TournamentRound> rounds = new List<TournamentRound>();
        ...
        public void SingleEliminationHandlesManyCompetitorsWell()
        {
            ...
            for (int i = 30; i <= 40; i++)
            {
                ...
                {
                    teamNames.Add(team.Id, "Team#" + team.Id);
                }
        ...
        public void SingleEliminationHandlesOutOfOrderCompetitors()
        {
            for (int i = 0; i < 10; i++)
            {
                ...
                var teams = (from k in teamNames.Keys
                             orderby k
                             select new User(k, r.Next(1000))).ToList();
        ...
        public void RunTournament(IPairingsGenerator pg, List<User> teams, List<TournamentRound> rounds, bool allowTies, TournamentNameTable nameTable)
        {
        ...
        private IEnumerable<User> CreateTeams(int teamCount)
        {
            for (int i = 1; i <= teamCount; i++)
            {
                yield return new User(i, 1000);
            }

            yield break;
        }

        private IEnumerable<TournamentRound> RounRobinBuildAllPairings(IEnumerable<User> teams, IPairingsGenerator rrpg)
        {
            List<TournamentRound> rounds = new List<TournamentRound>();
        ...
        private void DisplayTournamentRound(TournamentRound round)
        {
            ...
            foreach (TournamentPairing pairing in round.Pairings)
            {
                ...
                {
                    Console.Write(" " + teamScore.Team.Id.ToString().PadLeft(2));
                }
                Console.Write("\n");
            }
            Console.WriteLine();
        ...
        private void DisplayTournamentRankings(IEnumerable<TournamentRanking> rankings)
        {
            ...
            {
                Console.WriteLine("Rank " + ranking.Rank.ToString().PadRight(2) + "  Team " + ranking.Team.Id.ToString().PadRight(2) + "  Reason: " + ranking.ScoreDescription);
            }
            Console.WriteLine();
        }
    }
